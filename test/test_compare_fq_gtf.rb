...
class TestCompareFQGTF < Test::Unit::TestCase
  ...
  def test_statistics()
    ...
    assert_equal(compare_obj.weak_TP,[627, 359, 159, 63, 23, 11, 3, 4, 0, 5, 0])
    assert_equal(compare_obj.all_FP,[268, 91, 34, 4, 0, 0, 0, 1, 0, 0, 0])
    assert_equal(compare_obj.false_negatives,[41, 10, 9, 9, 8, 0, 2, 0, 0, 3, 0])
    assert_equal(compare_obj.strong_TP_by_cov,[72, 67, 63, 36, 13, 6, 1])
    assert_equal(compare_obj.weak_TP_by_cov,[627, 462, 218, 75, 22, 10, 1])
    assert_equal(compare_obj.all_FP_by_cov,[130, 66, 28, 14])
    assert_equal(compare_obj.false_negatives_by_cov,[41, 41, 24, 6, 1])
  end
